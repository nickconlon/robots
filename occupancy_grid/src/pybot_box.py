...
def sensorCallback(data):
  ...
  if(state.x != None and state.y != None): #so we don't try to map before we get state information
    ...
    occupancyGridMappingBoundingBox(x, z, beamwidth, beammax, data.range_max)
...
#
# Run the occupancy grid mapping algorithm that only looks at a bounding box
# around the current point
#
def occupancyGridMappingBoundingBox(cp, z, beamwidth, beammax, zmax):
  thetas = getSensorAngles()
  heading = thetas[2]

  for yy in np.arange(-5, 5, 1.0/SCALE_FACTOR):
    for xx in np.arange(-5, 5, 1.0/SCALE_FACTOR):
      
      # get the x,y we want to look at based on current position
      x = (cp.getX()+xx)
      y = (cp.getY()+yy)

      if(y >= 0 and x >=0 and y*SCALE_FACTOR < GRID_MAX_Y and x*SCALE_FACTOR < GRID_MAX_X):
